...
/// }
/// ```
public final class ReduceMotionObserver {

    // MARK: - Shared Instance

    public static let shared = ReduceMotionObserver()

    // MARK: - Published State

    /// `true` when the system *Reduce Motion* accessibility setting is on.
    public private(set) var isEnabled: Bool = UIAccessibility.isReduceMotionEnabled
