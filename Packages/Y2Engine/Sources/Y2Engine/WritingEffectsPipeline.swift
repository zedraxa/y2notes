...
public final class WritingEffectsPipeline {

    // MARK: - Tuning

    private enum Tuning {
        // ── Glow Pen ────────────────────────────────────────────────────
        public static let glowLayerSize: CGFloat          = GlowPenParams.diameter
        public static let glowBaseOpacity: Float          = Float(GlowPenParams.baseOpacity)
        public static let glowMaxOpacity: Float           = Float(GlowPenParams.maxOpacity)
        public static let glowFadeOutDuration: CFTimeInterval = GlowPenParams.fadeOutDuration

        // ── Neon Ink ─────────────────────────────────────────────────────
        public static let neonShadowRadius: CGFloat       = NeonInkParams.shadowRadius
        public static let neonShadowOpacity: Float        = NeonInkParams.shadowOpacity
        public static let neonFilter: String              = NeonInkParams.compositingFilter

        // ── Ink Trail ────────────────────────────────────────────────────
        public static let trailLineWidth: CGFloat         = InkTrailFadeParams.lineWidth
        public static let trailStartOpacity: CGFloat      = InkTrailFadeParams.startOpacity
        public static let trailMaxAge: TimeInterval       = InkTrailFadeParams.maxAge
        public static let trailPruneInterval: TimeInterval = InkTrailFadeParams.pruneInterval

        // ── Gradient Ink ─────────────────────────────────────────────────
        public static let gradientVelocityCeiling: CGFloat = VelocityThicknessParams.velocityCeiling

        // ── Stroke Taper ─────────────────────────────────────────────────
        /// Diameter of the taper dot rendered at stroke start/end (points).
        public static let taperDotDiameter: CGFloat       = 6.0
        /// Alpha of the taper dot at its most visible.
        public static let taperDotPeakOpacity: Float      = 0.55
        /// Duration of the taper dot fade animation (seconds).
        public static let taperFadeDuration: CFTimeInterval = 0.18

        // ── Ink Pooling ──────────────────────────────────────────────────
        /// Velocity (pts/s) below which ink pooling is triggered.
        public static let poolingVelocityThreshold: CGFloat = 80.0
        /// Maximum extra radius added by pooling (fraction of glow base size).
        public static let poolingMaxRadiusScale: CGFloat  = 1.8
        /// Duration of the pooling expand/contract animation (seconds).
        public static let poolingAnimDuration: CFTimeInterval = 0.12

        // ── General ──────────────────────────────────────────────────────
        public static let transitionDuration: CFTimeInterval = 0.25
    }
    ...
    /// active.
    public func attach(to canvasView: UIView) {
    ...
    public func configure(config: WritingEffectConfig, color: UIColor) {
        self.config = config
        self.inkColor = color
        syncNeonOverlay()
        if effectiveConfig.hasAdvancedEffects {
    ...
    public func onStrokeBegan(at point: CGPoint, pressure: CGFloat = 1.0) {
        lastNibPoint = point
